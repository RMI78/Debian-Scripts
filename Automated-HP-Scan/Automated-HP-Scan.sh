...
if [ ! -d $path_file ]; then
  read "this directory doesn't exist, do you want to create it ? (y for yes, n for no): " create_dir
...
if [ $output_format = "pdf" ];then
  read 'Do you want unite all your scan into 1 pdf ? (y for yes, n for no): ' unite_pdf
  if [ $unite_pdf = 'y' ];then
